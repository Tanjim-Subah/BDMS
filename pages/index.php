...
$low_stock_qry = "SELECT bloodgroup FROM blood GROUP BY bloodgroup HAVING count(*) < 2";
$low_stock_result = mysqli_query($conn, $low_stock_qry);

$low_stock_bloodgroups = [];
while ($row = mysqli_fetch_assoc($low_stock_result)) {
    $low_stock_bloodgroups[] = $row['bloodgroup'];
}

$low_stock_js_array = json_encode($low_stock_bloodgroups);
?>
...
<head>
	...
	<script type="text/javascript">  
           ...
           window.onload = function() {
            var lowStockBloodGroups = <?php echo $low_stock_js_array; ?>;
            if (lowStockBloodGroups.length > 0) {
                alert("Warning: The following blood groups have less than 2 units available: " + lowStockBloodGroups.join(", "));
            }
        };
           </script>
